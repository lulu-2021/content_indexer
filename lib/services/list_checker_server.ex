defmodule ContentIndexer.Services.ListCheckerServer do
 ...
 @doc """
    start a worker - ListCheckerWorker for each list of tokens being processed

   we pass it the index in the list of token lists to validate all have been processed, the word/token
    we are checking and the tokens we are checking against

   iex> ListCheckerServer.start_worker(list_index, word, tokens)
  """
  def start_worker(server_pid, list_index, word, tokens) do
    if list_index < @debug_iterations, do: debug_worker_start(list_index, word, tokens)
    pid = ListCheckerWorker.start
    send(pid, {:list, "#{list_index},#{word},#{tokens}", server_pid})
  end
